...
public class ParallelProcessing {
    ...
    public List<Integer> solve(Graph graph, int threadCount) {
        this.graph = graph;
        initData();
        List<Thread> threads = new ArrayList<>();
        for (int i = 0; i < threadCount; i++) {
            PriorityQueue<Node> localQueue = new PriorityQueue<>();
         //   System.out.println("localQueue :: "+ localQueue);
            queues.add(localQueue);
        }
        ReduceRunnable reduceRunnable = new ReduceRunnable(queues, isFinished, visited, currentNode);
        CyclicBarrier cyclicBarrier = new CyclicBarrier(threadCount, reduceRunnable);
        int start;
        int end = 0;
        int chunk = graph.getNumberOfNodes() / threadCount;
        int leftover = graph.getNumberOfNodes() % threadCount;
